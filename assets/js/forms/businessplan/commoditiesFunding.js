 ...
 const commoditiesEC={};
  ...
  function populateProgramEvents(dataValues) {
    ...
    // $('#push-button').append(`<button ${tei.disabled ? 'disabled readonly': ''} class="btn btn-success p-2 my-2" onclick="event.preventDefault();pushToAnnualReport()">Submit Business Plan</button>`)
     ...
     console.trace("=====")
     $('#push-button').append(`<button ${true ? 'disabled readonly' : ''} class="btn btn-success p-2 my-2" id="submit-btn" onclick="event.preventDefault(); pushToAnnualReport(); this.style.backgroundColor='gray'; this.disabled=true;">Submit Business Plan</button>`);
      ...
      totalsRow += `
        ...
        <td>
          <div class="input-group">
            ...
            <input type="text" value="${commoditiesEC[year] ? formatNumberInput(commoditiesEC[year]) : ''}" class="form-control currency" disabled readonly>
          </div>
        </td>
        <td>
          <div class="input-group">
            ...
            <input type="text" style="background:${variation >=0 ? '#C1E1C1 !important':'#FAA0A0 !important'}" value="${formatNumberInput(variation)}" id="${dataElements.sourceCommodities['variation']}-${year}" class="form-control difference-${year} currency" disabled readonly>
          </div>
        </td>
      </tr>
      `;
