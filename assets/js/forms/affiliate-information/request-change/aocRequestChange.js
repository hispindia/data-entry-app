import { attributes, optionSet, orgUnit, programs, source, trackedEntityType } from "../../../constant.js";
...
import { displayAffiliate } from "./affiliate.js";
...
const handleAocRequestChange = async (userConfig) => {
  ...
  async function fetchAffiliateList() {
    ...
    const affiliateRows = displayAffiliateList(programUINControl.programTrackedEntityAttributes, affiliateList?.trackedEntities,'aoc');
    ...
    document.getElementById("tfoot-affiliate").innerHTML = affiliateRows.tfootAffiliate;
    
    document.querySelectorAll('.action-button').forEach(button => {
      button.addEventListener('click', async function (e) {
        const affiliate = e.currentTarget.dataset.affiliate;
        const affiliateView = await displayAffiliate(affiliate);
        document.querySelector('#profile-modal').style.display = "block";
      });
    });

    document.querySelector('#schedule-all').addEventListener('click', function(e) {
      if(e.target.checked) {
    ...
    document.querySelector('#submit-acuity').addEventListener('click', async function() {
      const scheduleAll = document.querySelector('#schedule-all').checked;
      const checkList = document.querySelectorAll('.check-list');
      const checkedAffiliates = [];
      if(scheduleAll) {
        checkList.forEach(checkbox => {
          if(checkbox.dataset.affiliate){
            checkedAffiliates.push(checkbox.dataset.affiliate);
          }
        })
      } else {
        checkList.forEach(checkbox => {
          if(checkbox.dataset.affiliate && checkbox.target.checked){
            checkedAffiliates.push(checkbox.dataset.affiliate);
          }
        })
      }
      ...
      for (const affiliate of checkedAffiliates) {
          const affiliateVal = affiliateList.trackedEntities.find(
              entity => entity.trackedEntity === affiliate
          );

          if (affiliateVal?.orgUnit) {
              trackedEntities.push({
                  trackedEntity: affiliate,
                  orgUnit: affiliateVal.orgUnit,
                  trackedEntityType,
                  attributes: [
                      {
      ...
      await Promise.all(
          checkedAffiliates.map(affiliate =>
              dataApi.dataStoreDelete("accuityResponse", affiliate)
          )
      );
